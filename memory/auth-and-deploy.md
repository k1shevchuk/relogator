...
- Updated on 2026-07-01: Supabase confirmation and recovery email templates should use `{{ .RedirectTo }}&token_hash=...` for both the button and the fallback copy link. Do not rebuild auth email links from `{{ .SiteURL }}`; stale `SiteURL`-based links caused confusing local redirects in old emails.
